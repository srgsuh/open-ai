...
async def response_validation_handler(request: Request, exc: Exception) -> JSONResponse:
    if isinstance(exc, ValidationError):
        if request.scope.get("response_validation", False):
            return JSONResponse(
                status_code=500,
                content={
                    "detail": jsonable_encoder(exc.errors())
        ...
        else:   # for request validation (generated by Depends)
            logger.debug(f"response_validation_handler. Catch validation error, thrown by dependency")
            return JSONResponse(
                status_code=400,
                content={
                    "detail": jsonable_encoder(exc.errors())
